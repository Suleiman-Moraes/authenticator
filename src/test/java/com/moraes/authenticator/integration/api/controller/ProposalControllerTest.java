...
/**
 * This class definition defines a test integration class for the {@link ProposalController}
 */
...
public class ProposalControllerTest extends AbstractIntegrationTest {
	...
	public static void setup() {
		...
		specification = new RequestSpecBuilder().setBasePath(BASE_URL).setPort(TestConfig.SERVER_PORT)
	...
	void givenKeyWhenFindByKeyThenReturnProposalDTO() throws Exception {
		...
		assertEquals(condition.getNumberInstallments(), conditionResponse.getNumberInstallments(),
				"NumberInstallments not equal");
		assertEquals(condition.getValueInstallments().doubleValue(),
				conditionResponse.getValueInstallments().doubleValue(), "ValueInstallments not equal");
		assertEquals(condition.getBeginningInstallment(), conditionResponse.getBeginningInstallment(),
	...
	void givenProposalFilterDTOWithPaginateFalseWhenFindAllThenReturnPage() throws Exception {
		...
		final Response response = given().spec(specification).header(AUTHORIZATION, ACCESS_TOKEN)
				.params(JsonObjectUtil.convertObjectToMap(proposalFilter)).when().get();

		response.then().statusCode(200);

		final Map<String, Object> page = mapper.readValue(response.getBody().asString(), new TypeReference<>() {
		});
		final List<ProposalListDTO> content = JsonObjectUtil.convertMapsToListOfSomething(
				(List<Map<String, Object>>) page.get("content"), ProposalListDTO.class);
		...
		assertTrue(Integer.parseInt(page.get("totalElements").toString()) > 0,
				"Total elements is not greater than zero");
	...
	@Test
	@Order(4)
	@DisplayName("JUnit Integration test Given ProposalDTO And key When update Then return no content")
	void givenProposalDTOAndKeyWhenUpdateThenReturnNoContent() throws Exception {
		dto.setVpl(BigDecimal.valueOf(1500));
		dto.setValue(BigDecimal.valueOf(15000));
		dto.getConditions().remove(dto.getConditions().size() - 1);
		dto.getConditions().get(0).setFrequency(FrequencyEnum.MONTHLY);
		dto.getConditions().get(0).setValueInstallments(BigDecimal.valueOf(1500));
        dto.getEnterprise().setConstructionName("New Construction Name");
        dto.getEnterprise().setName("New Enterprise Name");
        dto.getEnterprise().setUnit(null);
        dto.getEnterprise().setSizeM2(BigDecimal.valueOf(1000));

		Response response = given().spec(specification).pathParam(NAME_KEY, key).contentType(APPLICATION_JSON)
				.header(AUTHORIZATION, ACCESS_TOKEN).body(dto).when().put(PATH_KEY);
		...
		assertEquals(dto.getVpl().doubleValue(), dtoResponse.getVpl().doubleValue(), "Vpl not equal");
		assertEquals(dto.getValue().doubleValue(), dtoResponse.getValue().doubleValue(), "Value not equal");
        assertEquals(dto.getEnterprise().getConstructionName(), dtoResponse.getEnterprise().getConstructionName(),
                "ConstructionName not equal");
        assertEquals(dto.getEnterprise().getName(), dtoResponse.getEnterprise().getName(), "Name not equal");
        assertNull(dtoResponse.getEnterprise().getUnit(), "Unit not null");
        assertEquals(dto.getEnterprise().getSizeM2().doubleValue(), dtoResponse.getEnterprise().getSizeM2().doubleValue(),
                "SizeM2 not equal");
		assertEquals(dto.getConditions().size(), dtoResponse.getConditions().size(), "Conditions not equal");
        assertEquals(dto.getConditions().get(0).getFrequency(), dtoResponse.getConditions().get(0).getFrequency(),
				"Frequency not equal");
		assertEquals(dto.getConditions().get(0).getValueInstallments().doubleValue(),
				dtoResponse.getConditions().get(0).getValueInstallments().doubleValue(), "ValueInstallments not equal");
		dto = dtoResponse;
	}

	public static Response insert(RequestSpecification specification, ProposalDTO proposalDTO) {
		final Response response = given().spec(specification).basePath(BASE_URL).contentType(APPLICATION_JSON)
				.header(AUTHORIZATION, ACCESS_TOKEN).body(proposalDTO).when().post();
	...
	private static Response findByKey(Long key) {
		return given().spec(specification).pathParam(NAME_KEY, key).header(AUTHORIZATION, ACCESS_TOKEN).when()
				.get(PATH_KEY);
	}
}
