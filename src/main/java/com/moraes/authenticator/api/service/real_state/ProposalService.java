...
import com.moraes.authenticator.api.mapper.IProposalMapper;
...
public class ProposalService implements IProposalService {
	...
	public Long insert(ProposalDTO proposalDTO) {
		...
		proposal.getEnterprise().setKey(enterpriseService.insert(proposal.getEnterprise(),
				proposalDTO.getEnterprise().getConstructionName()));
	...
	public Proposal findByKey(Long key) {
		return repository.findByKeyAndEnterpriseConstructionCompanyKey(key,
				SecurityUtil.getCompanyPrincipalOrThrow().getKey()).orElseThrow(ResourceNotFoundException::new);
	}
	...
	public void update(ProposalDTO proposalDTO, Long key) {
		...
		proposalMapper.updateFromProposalDTO(proposal, proposalDTO);

		enterpriseService.update(proposal.getEnterprise(), proposalDTO.getEnterprise());
		conditionService.updateAll(proposal.getConditions(), proposalDTO.getConditions(), proposal.getKey());
